...
shorecomps <- function(page = NULL,
  ...
  utils::write.csv(afs,
    file = file.path(
      mydir, "Catches", "Comps", "Shoreside.Age.Only",
      "shoresideAgeComps.csv"
    ), row.names = FALSE
  )
  utils::write.csv(
    row.names = FALSE,
    ...
    reshape(
      aggregate(FREQ ~ SOURCE_AGID + FISH_AGE_YEARS_FINAL + SAMPLE_YEAR,
  ...
  utils::write.csv(
    ...
    reshape(
      aggregate(FREQ ~ GRID + FISH_AGE_YEARS_FINAL + SAMPLE_YEAR,
        data = page.worked, length
      ),
