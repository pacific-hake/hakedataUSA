...
plotMaturity.fn <- function(x, theFactor, cols, factorLab=sort(as.character(unique(x[,theFactor]))),
                            alpha=0.6, inch=0.1, ht=0.1, labHadj=0.3, legYinter=1, legx="right", legy=NULL, legTitle=NULL) {
...
#        segments(out$len,-i*ht,out$len,out$tots/(max(out$tots)/ht)-i*ht,col=cols[i],lwd=3)  #this will plot each factor to have its own max (and at least 1 segment reaching the top)
        ...
        tmp <- plotPredMaturityLength(16:80, dat[[i]],
                           start=c(-6.2,0.179,0.95), low=c(-Inf,-Inf,0.7), upp=c(Inf,Inf,1),
                           col=cols[i], lwd=3)
    }
